...
var EmptyIntElementsError error = errors.New("empty intElements")

func (s *intElements) Len() int {
	return len(*s)
}
func (s *intElements) IsNan(i int) bool {
	return (*s)[i].IsNaN()

}
func (s *intElements) Get(i int) interface{} {
	return ((*s)[i].Get()).(int64)
}
func (s *intElements) Head() string {
	length := len(*s)
	if length > 5 {
		length = 5
	}
...
func (s *intElements) Max() Element {
	if s.Len() == 0 {
...
var _ Elements = (*intElements)(nil)
